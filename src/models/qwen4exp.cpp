...
ggml_tensor * llama_model_qwen4exp::paw_x3_create(
        llama_model_loader & ml,
        const LLM_TN_IMPL & tnv,
        bool required) {
    ...
    if (meta == nullptr) {
        ...
        throw std::runtime_error("qwen4exp: missing PAW X3 tensor '" + name + "'");
    }
    ...
    switch (ggml_n_dims(meta)) {
        ...
        default:
            throw std::runtime_error("qwen4exp: unexpected rank for PAW X3 tensor '" + name + "'");
    }
}
...
void llama_model_qwen4exp::load_arch_tensors(llama_model_loader & ml) {
    ...
    if (hparams.ple_n_heads > 0) {
        ...
        const std::string ple_q8_name    = tn(LLM_TENSOR_PER_LAYER_TOKEN_EMBD, "q8").str();
        const auto * ple_weight = ml.get_tensor_meta(ple_name.c_str());
        const auto * ple_q8     = ml.get_tensor_meta(ple_q8_name.c_str());
        if (ple_weight && ple_q8) {
            throw std::runtime_error("qwen4exp: GGUF contains both BF16 and PAW Q8 PLE tables");
        }
        const auto * ple_tensor = ple_q8 ? ple_q8 : ple_weight;
        if (ple_tensor) {
            if (ple_tensor->ne[1] < ple_rows) {
                throw std::runtime_error(format("%s has %" PRId64 " rows, too few for the PLE head ranges (%" PRId64 ")",
                                                ple_tensor->name, ple_tensor->ne[1], ple_rows));
            }
            ple_rows = ple_tensor->ne[1];
        }

        if (ple_q8) {
            per_layer_tok_embd = create_tensor(tn(LLM_TENSOR_PER_LAYER_TOKEN_EMBD, "q8"),
                                               { hparams.ple_head_dim, ple_rows }, TENSOR_READ_LAZY);
            per_layer_tok_embd_scale = create_tensor(tn(LLM_TENSOR_PER_LAYER_TOKEN_EMBD, "scale"),
                                                     { 1, ple_rows }, TENSOR_READ_LAZY);
        } else {
    ...
    for (int il = 0; il < (int) hparams.n_layer_all; ++il) {
        ...
        auto & x3 = paw_x3_layers[il];
        x3.gate.trellis = paw_x3_create(ml, tn(LLM_TENSOR_FFN_GATE_EXPS, "m3_trellis", il), false);
        if (x3.gate.trellis) {
            auto load_proj = [&](paw_x3_proj & proj, llm_tensor base) {
                if (!proj.trellis) {
                    proj.trellis = paw_x3_create(ml, tn(base, "m3_trellis", il), true);
                }
                proj.meta = paw_x3_create(ml, tn(base, "m3_meta", il), true);
                proj.suh  = paw_x3_create(ml, tn(base, "m3_suh", il), true);
                proj.svh  = paw_x3_create(ml, tn(base, "m3_svh", il), true);
            };
            load_proj(x3.gate, LLM_TENSOR_FFN_GATE_EXPS);
            load_proj(x3.up,   LLM_TENSOR_FFN_UP_EXPS);
            load_proj(x3.down, LLM_TENSOR_FFN_DOWN_EXPS);
        } else {
        ...
        // Optional: only this fork's own MTP export carries a per-block
...
llama_model_qwen4exp::graph_mtp::graph_mtp(const llama_model_qwen4exp & model, const llm_graph_params & params) :
    ...
    GGML_ASSERT((layer.nextn.hc_head_norm || model.hc_head_norm) &&
                "MTP block needs nextn.hc_head_norm or the model-level output_hc_norm");
    ...
    // A head exported without its own hc mix uses the model's, as the
    // reference qwen4exp MTP graph does.
    const bool hc_per_block = layer.nextn.hc_head_norm != nullptr;
    cur = build_hc_mix(res_hc,
            hc_per_block ? layer.nextn.hc_head_norm : model.hc_head_norm,
            hc_per_block ? layer.nextn.hc_head_down : model.hc_head_down,
            hc_per_block ? layer.nextn.hc_head_up   : model.hc_head_up,
            nullptr, nullptr, -1);
...
ggml_tensor * llama_model_qwen4exp::graph::build_layer_ffn(ggml_tensor * cur, const int il) {
    ...
    const auto & x3 = model.paw_x3_layers[il];
    if (x3.gate.trellis) {
        ...
        cb(selected->src[0], "ffn_moe_argsort", il);
        ...
        ggml_tensor * weights_sum = ggml_clamp(ctx0, ggml_sum_rows(ctx0, weights), 6.103515625e-5, INFINITY);
        ...
        ggml_tensor * proj[12] = {
            x3.gate.trellis, x3.gate.meta, x3.gate.suh, x3.gate.svh,
            x3.up.trellis,   x3.up.meta,   x3.up.suh,   x3.up.svh,
            x3.down.trellis, x3.down.meta, x3.down.suh, x3.down.svh,
        };
        moe_out = ggml_paw_x3_moe(ctx0, cur, selected, weights, proj);
    } else {
...
ggml_tensor * llama_model_qwen4exp::graph::build_inp_ple(
    ...
    if (model.per_layer_tok_embd_scale) {
        ggml_tensor * scale = ggml_get_rows(ctx0, model.per_layer_tok_embd_scale, rows);
        emb = ggml_mul(ctx0, emb, scale);
    }
