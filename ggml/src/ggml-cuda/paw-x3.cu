...
// ---------------------------------------------------------------------------------------------------------
// Fused routed-expert SwiGLU FFN, ported from exllamav3 quant/exl3_moe_kernel.cuh + quant/exl3_moe.cu
// (MIT, ref 1.4.2). One launch runs gate/up GEMM, SiLU gate, down GEMM and the weighted scatter for
// every expert with at most max_tokens_per_expert rows; groups of SMs each walk the expert list and
// draw experts from a self-resetting ticket scheduler.
// Differences from the reference: the rate is per expert (tables_k) instead of one K per projection,
// so the K switch runs per expert; routing weights are fp32 and token/count tables int32; SiLU only;
// tiles are fixed at 32x128 (any hidden/intermediate size divisible by 128); locks come from the pool.

#define X3M_SMS_PER_EXPERT     8
#define X3M_MAX_SMS_PER_EXPERT 32
#define X3M_MAX_GROUPS         64
#define X3M_TILESIZE_K         32
#define X3M_TILESIZE_N         128
#define X3M_SH_STAGES          3
#define X3M_FRAG_STAGES        3
#define X3M_BLOCK_THREADS      (X3G_BASE_THREADS * X3M_TILESIZE_K / 16)
// locks: [tile locks, X3M_LOCKS_PER_GROUP per group | barrier counter+sense per group | scheduler]
#define X3M_LOCKS_PER_GROUP    64
#define X3M_BARRIER_OFFSET     (X3M_MAX_GROUPS * X3M_LOCKS_PER_GROUP)
#define X3M_SCHED_OFFSET       (X3M_BARRIER_OFFSET + 2 * X3M_MAX_GROUPS)
#define X3M_LOCK_INTS          (X3M_SCHED_OFFSET + 2 + X3M_MAX_GROUPS)

// ptx.cuh group_barrier: all blocks of a group meet here; the last arrival flips the sense
__device__ inline void x3m_group_barrier(int group_id, int group_size, int * barrier_counters_sense)
{
    __syncthreads();
    ...
    // hold every thread of the block until thread 0 has cleared the group barrier; without this the
    // other threads start the next stage while other blocks of the group are still writing
    __syncthreads();
}

__device__ inline half4 x3m_had4(half4 v, const float r_scale)
{
    int t = threadIdx.x & 31;
    float v0 = __half2float(__low2half(v.x));
    float v1 = __half2float(__high2half(v.x));
...
__device__ inline half2 x3m_silu(const half2 & x)
{
...
// hadamard_inner.cuh had_hf_r_128_guad_inner (SiLU): o <- in_had(silu(out_had(g)) * out_had(u))
inline __device__
void x3m_had_guad_inner(const half * __restrict__ input_ptr_g, const half * __restrict__ input_ptr_u,
                        half * __restrict__ output_ptr,
                        const half * __restrict__ post_scale_g, const half * __restrict__ post_scale_u,
                        const half * __restrict__ pre_scale_d, const float r_scale)
{
    int t = threadIdx.x & 31;
    int i = blockIdx.y * 32 + t;

    half4 vu = x3m_had4(((half4 *) input_ptr_u)[t], r_scale);
    half4 scales_u = ((half4 *) post_scale_u)[i];
    vu.x = __hmul2(vu.x, scales_u.x);
    vu.y = __hmul2(vu.y, scales_u.y);

    half4 vg = x3m_had4(((half4 *) input_ptr_g)[t], r_scale);
    half4 scales_g = ((half4 *) post_scale_g)[i];
    vg.x = __hmul2(vg.x, scales_g.x);
    vg.y = __hmul2(vg.y, scales_g.y);

    vg.x = __hmul2(x3m_silu(vg.x), vu.x);
    vg.y = __hmul2(x3m_silu(vg.y), vu.y);

    half4 scales_d = ((half4 *) pre_scale_d)[i];
    vg.x = __hmul2(vg.x, scales_d.x);
    vg.y = __hmul2(vg.y, scales_d.y);

    ((half4 *) output_ptr)[t] = x3m_had4(vg, r_scale);
}

// hadamard_inner.cuh had_hf_r_128_d_inner: o += float(out_had(i)), atomic
inline __device__
void x3m_had_d_inner(const half * __restrict__ input_ptr, float * __restrict__ output_ptr,
                     const half * __restrict__ post_scale, const float r_scale)
{
    int t = threadIdx.x & 31;
    half4 v = ((half4 *) input_ptr)[t];
    ...
    // reshuffle in shmem for a coalesced store with atomicAdd
    extern __shared__ float x3m_temp_shared[];
    int warp_id = threadIdx.x / 32;
    float * sh = x3m_temp_shared + warp_id * 128;
    sh[t * 4 + 0] = h0;
    sh[t * 4 + 1] = h1;
    sh[t * 4 + 2] = h2;
    sh[t * 4 + 3] = h3;
...
// tables_* hold 3 * num_experts entries: [gate | up | down]
__global__ __launch_bounds__(X3M_BLOCK_THREADS)
void x3m_moe_kernel(
    ...
    const uint16_t ** __restrict__ tables_trellis,
    const half ** __restrict__ tables_suh,
    const half ** __restrict__ tables_svh,
    const int * __restrict__ tables_k,
    ...
    int * __restrict__ locks)
{
    ...
    const int block_threads = X3M_BLOCK_THREADS;
    ...
    const float r_scale = 0.088388347648f;
    ...
    int * barrier_counters_sense = locks + X3M_BARRIER_OFFSET;
    int * sched = locks + X3M_SCHED_OFFSET;
    locks += group_idx * X3M_LOCKS_PER_GROUP;

    // active experts are numbered in scan order; a group processes the active expert matching its
    // ticket and draws the next unclaimed ticket when done
    int ticket = group_idx;

    auto gemm = [&] (const half * in_addr, half * out_addr, const uint16_t * trellis, const int K,
                     int size_m, const int size_k, const int size_n)
    {
        while (size_m > 0)
        {
            #define X3M_GEMM(k_) x3g_gemm_inner<k_, false, 2, 16, X3M_TILESIZE_K, X3M_TILESIZE_N, \
                                                X3M_SH_STAGES, X3M_FRAG_STAGES, false>          \
                (in_addr, trellis, out_addr, X3G_MIN(size_m, 16), size_k, size_n, locks, nullptr)
            switch (K)
            {
                case 1: X3M_GEMM(1); break;
                case 2: X3M_GEMM(2); break;
                case 3: X3M_GEMM(3); break;
                case 4: X3M_GEMM(4); break;
            }
            #undef X3M_GEMM
            in_addr += 16 * size_k;
            out_addr += 16 * size_n;
    ...
    for (int expert_idx = 0; expert_idx < num_experts; ++expert_idx)
    {
        ...
        const int eg = expert_idx;
        const int eu = num_experts + expert_idx;
        const int ed = 2 * num_experts + expert_idx;

        // gather + input hadamard for g, u
        {
            const int warps_per_token = hidden_dim / 128;
            const int total_warps = token_count * warps_per_token;
            const int * top_x = token_sorted + start;
            for (int warp_idx = warp_idx0; warp_idx < total_warps; warp_idx += warps_per_group)
            {
                ...
                had_hf_r_128_inner<true, false>(in_ptr, temp_state_g + 128 * warp_idx,
                                                tables_suh[eg] + 128 * token_off, r_scale);
                had_hf_r_128_inner<true, false>(in_ptr, temp_state_u + 128 * warp_idx,
                                                tables_suh[eu] + 128 * token_off, r_scale);
            }
            x3m_group_barrier(group_idx, group_size, barrier_counters_sense);
        }

        gemm(temp_state_g, temp_intermediate_g, tables_trellis[eg], tables_k[eg], token_count, hidden_dim, intermediate_dim);
        gemm(temp_state_u, temp_intermediate_u, tables_trellis[eu], tables_k[eu], token_count, hidden_dim, intermediate_dim);
        x3m_group_barrier(group_idx, group_size, barrier_counters_sense);
        ...
        // output hadamard for g, u + SiLU gate + input hadamard for d
        {
            ...
            {
                int token_off = warp_idx % warps_per_token;
                x3m_had_guad_inner(temp_intermediate_g + 128 * warp_idx,
                                   temp_intermediate_u + 128 * warp_idx,
                                   temp_intermediate_g + 128 * warp_idx,
                                   tables_svh[eg] + 128 * token_off,
                                   tables_svh[eu] + 128 * token_off,
                                   tables_suh[ed] + 128 * token_off,
                                   r_scale);
            }
            x3m_group_barrier(group_idx, group_size, barrier_counters_sense);
        }

        gemm(temp_intermediate_g, temp_state_g, tables_trellis[ed], tables_k[ed], token_count, intermediate_dim, hidden_dim);
        x3m_group_barrier(group_idx, group_size, barrier_counters_sense);
        ...
        // output hadamard for d + weighted scatter add
        {
            ...
            const int total_warps = token_count * warps_per_token;
            const int * top_x = token_sorted + start;
            const float * weights = weight_sorted + start;
            for (int warp_idx = warp_idx0; warp_idx < total_warps; warp_idx += warps_per_group)
            {
                int token_idx = top_x[warp_idx / warps_per_token];
                float weight = weights[warp_idx / warps_per_token];
                int token_off = warp_idx % warps_per_token;
                float * out_ptr = output_state + token_idx * hidden_dim + token_off * 128;
                x3m_had_d_inner(temp_state_g + 128 * warp_idx, out_ptr,
                                tables_svh[ed] + 128 * token_off, r_scale * weight);
            }
        }

        // the end-of-expert barrier publishes the next ticket and protects the temp buffers
    ...
    // retire the group; the last group out resets the scheduler for the next launch
    if (block_idx == 0 && threadIdx.x == 0)
    {
...
// fallback lane for experts above the fused row limit: g <- silu(g) * u
__global__ static void x3m_swiglu_kernel(float * g, const float * u, int n)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) {
...
// dst[tok[r]] += w[r] * y[r]; rows of one expert have distinct tokens
__global__ static void x3m_scatter_add_kernel(float * dst, const float * y, const int * tok, const float * w, int n_embd)
{
    int r = blockIdx.y;
    int j = blockIdx.x * blockDim.x + threadIdx.x;
    if (j < n_embd) {
        dst[tok[r] * n_embd + j] += w[r] * y[r * n_embd + j];
    }
}
...
void ggml_cuda_op_paw_x3_mm(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    // The sq kernel covers outputs in whole 256-column blocks; exllamav3 refuses the int8
...
static const int32_t * x3_expert_meta(const ggml_tensor * meta, const ggml_tensor * trellis, int64_t ntiles,
                                      cudaStream_t stream) {
    struct meta_entry { const void * trellis; std::vector<int32_t> v; };
    static std::mutex meta_mtx;
    static std::unordered_map<const void *, meta_entry> meta_cache;
    ...
    if (me.trellis != trellis->data || me.v.size() != (size_t) (2 * n_expert)) {
        ...
        for (int64_t e = 0; e < n_expert; ++e) {
            const int32_t k   = me.v[2 * e];
            const int64_t off = me.v[2 * e + 1];
            GGML_ASSERT(k >= 1 && k <= 4);
            GGML_ASSERT(off >= 0 && off + 16 * k * ntiles <= trellis->ne[0]);
        }
    }
    return me.v.data();
}

// contiguous stack tensor over device memory, for driving the per-matrix x3 op on slices
static ggml_tensor x3_slice_tensor(ggml_type type, int64_t ne0, int64_t ne1, void * data) {
    ggml_tensor t;
    memset(&t, 0, sizeof(t));
    t.type  = type;
    t.ne[0] = ne0;
...
// y = W_e x through ggml_cuda_op_paw_x3_mm for expert e of a flat expert tensor
static void x3_expert_mm(ggml_backend_cuda_context & ctx, const ggml_tensor * trellis, const int32_t * kmeta,
                         const ggml_tensor * suh, const ggml_tensor * svh, int64_t e,
                         float * x, float * y, int64_t rows) {
    const int64_t n = suh->ne[0];
    const int64_t m = svh->ne[0];
    const int64_t k = kmeta[2 * e];
    ggml_tensor t_tr  = x3_slice_tensor(GGML_TYPE_I16, 16 * k, (m / 16) * (n / 16),
                                        (char *) trellis->data + (size_t) kmeta[2 * e + 1] * sizeof(int16_t));
    ggml_tensor t_suh = x3_slice_tensor(GGML_TYPE_F16, n, 1, (char *) suh->data + e * suh->nb[1]);
    ggml_tensor t_svh = x3_slice_tensor(GGML_TYPE_F16, m, 1, (char *) svh->data + e * svh->nb[1]);
    ggml_tensor t_x   = x3_slice_tensor(GGML_TYPE_F32, n, rows, x);
    ggml_tensor t_y   = x3_slice_tensor(GGML_TYPE_F32, m, rows, y);
    t_y.op     = GGML_OP_PAW_X3_MM;
    t_y.src[0] = &t_tr;
    t_y.src[1] = &t_suh;
    t_y.src[2] = &t_svh;
    t_y.src[3] = &t_x;
    ggml_cuda_op_paw_x3_mm(ctx, &t_y);
    CUDA_CHECK(cudaGetLastError());
}

// Routed experts with per-expert rates. Same structure as the sorted
// ggml_cuda_mul_mat_id path: rows are grouped by expert on the host, gathered,
// and each expert's batch runs through ggml_cuda_op_paw_x3_mm on slice tensors,
// so every dense x3 path (sq GEMV, tile GEMM, reconstruct + cuBLAS) is reused.
void ggml_cuda_op_paw_x3_mm_id(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    const ggml_tensor * trellis = dst->src[0];
    const ggml_tensor * meta    = dst->src[1];
    const ggml_tensor * suh     = dst->src[2];
    const ggml_tensor * svh     = dst->src[3];
    ...
    const int32_t * kmeta = x3_expert_meta(meta, trellis, ntiles, stream);

    std::vector<int32_t> ids_host(n_rows);
    CUDA_CHECK(cudaMemcpyAsync(ids_host.data(), ids->data, ggml_nbytes(ids), cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaStreamSynchronize(stream));

    // counting sort by expert; ids_to_sorted indexes the rows of x, which
    // broadcasts over the slot axis when x->ne[1] == 1
    std::vector<int32_t> rows_per_expert(n_expert + 1, 0);
    ...
    for (int64_t e = 0; e < n_expert; ++e) {
        rows_per_expert[e + 1] += rows_per_expert[e];
    }
    std::vector<int32_t> sort_host(2 * n_rows);
    {
        std::vector<int32_t> cursor(rows_per_expert.begin(), rows_per_expert.end() - 1);
        for (int64_t t = 0; t < n_tokens; ++t) {
            for (int64_t s = 0; s < n_used; ++s) {
                const int64_t i = t * n_used + s;
                const int32_t pos = cursor[ids_host[i]]++;
                sort_host[pos]          = (int32_t) (t * x->ne[1] + s % x->ne[1]);
                sort_host[n_rows + i]   = pos;
            }
        }
    }

    ggml_cuda_pool_alloc<int32_t> sort_dev(ctx.pool(), 2 * n_rows);
    CUDA_CHECK(cudaMemcpyAsync(sort_dev.get(), sort_host.data(), 2 * n_rows * sizeof(int32_t), cudaMemcpyHostToDevice, stream));
    const int32_t * ids_to_sorted   = sort_dev.get();
    const int32_t * ids_from_sorted = sort_dev.get() + n_rows;

    const size_t fs = sizeof(float);
    ggml_cuda_pool_alloc<float> x_sorted(ctx.pool(), n_rows * n);
    ggml_cuda_pool_alloc<float> y_sorted(ctx.pool(), n_rows * m);

    get_rows_cuda(x->data, GGML_TYPE_F32, ids_to_sorted, x_sorted.get(), GGML_TYPE_F32,
        n, x->nb[1], x->nb[2], x->nb[3],
        n_rows, 1, 1, sizeof(int32_t), n_rows * sizeof(int32_t), n_rows * sizeof(int32_t),
        n * fs, n_rows * n * fs, n_rows * n * fs, stream);
    CUDA_CHECK(cudaGetLastError());

    for (int64_t e = 0; e < n_expert; ++e) {
        const int32_t beg = rows_per_expert[e];
        const int32_t cnt = rows_per_expert[e + 1] - beg;
        if (cnt == 0) {
            continue;
        }
        x3_expert_mm(ctx, trellis, kmeta, suh, svh, e,
                     x_sorted.get() + (size_t) beg * n, y_sorted.get() + (size_t) beg * m, cnt);
    }

    get_rows_cuda(y_sorted.get(), GGML_TYPE_F32, ids_from_sorted, dst->data, GGML_TYPE_F32,
        m, m * fs, n_rows * m * fs, n_rows * m * fs,
        n_rows, 1, 1, sizeof(int32_t), n_rows * sizeof(int32_t), n_rows * sizeof(int32_t),
...
void ggml_cuda_op_paw_x3_moe(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    using namespace paw_x3;

    const ggml_tensor * x       = dst->src[0];
    const ggml_tensor * ids     = dst->src[1];
    const ggml_tensor * weights = dst->src[2];
    // per projection p (0 gate, 1 up, 2 down): trellis, meta, suh, svh
    auto P = [&](int p, int i) { return dst->src[3 + 4 * p + i]; };

    const int64_t n        = x->ne[0];
    const int64_t ni       = P(0, 3)->ne[0];
    const int64_t n_expert = P(0, 2)->ne[1];
    const int64_t n_used   = ids->ne[0];
    const int64_t n_tokens = ids->ne[1];
    const int64_t n_rows   = n_used * n_tokens;
    const size_t  fs       = sizeof(float);
    ...
    ggml_cuda_pool_alloc<float> weight_sorted(ctx.pool(), n_rows);
    get_rows_cuda(weights->data, GGML_TYPE_F32, flat_sorted_dev, weight_sorted.get(), GGML_TYPE_F32,
        1, fs, n_rows * fs, n_rows * fs,
        n_rows, 1, 1, sizeof(int32_t), n_rows * sizeof(int32_t), n_rows * sizeof(int32_t),
        fs, n_rows * fs, n_rows * fs, stream);
    CUDA_CHECK(cudaGetLastError());
    ...
    if (num_active > 0) {
        // pointer and rate tables per layer, persistent on the device (model data never moves)
        struct x3m_tables { const void * key[3]; void * dev; };
        static std::mutex tab_mtx;
        static std::unordered_map<const void *, x3m_tables> tab_cache;
        void * tables_dev;
        {
            ...
            if (tb.key[0] != P(0, 0)->data || tb.key[1] != P(1, 0)->data || tb.key[2] != P(2, 0)->data) {
                ...
                for (int p = 0; p < 3; ++p) {
                    for (int64_t e = 0; e < n_expert; ++e) {
                        const size_t j = p * n_expert + e;
                        ptrs[j]          = (char *) P(p, 0)->data + (size_t) kmeta[p][2 * e + 1] * sizeof(int16_t);
                        ptrs[np + j]     = (char *) P(p, 2)->data + e * P(p, 2)->nb[1];
                        ptrs[2 * np + j] = (char *) P(p, 3)->data + e * P(p, 3)->nb[1];
                        ks[j]            = kmeta[p][2 * e];
                    }
                }
                if (tb.dev == nullptr) {
                    CUDA_CHECK(cudaMalloc(&tb.dev, 3 * np * sizeof(void *) + np * sizeof(int32_t)));
                }
                CUDA_CHECK(cudaMemcpyAsync(tb.dev, ptrs.data(), 3 * np * sizeof(void *), cudaMemcpyHostToDevice, stream));
                CUDA_CHECK(cudaMemcpyAsync((char *) tb.dev + 3 * np * sizeof(void *), ks.data(), np * sizeof(int32_t),
        ...
        const size_t np = 3 * n_expert;
        const uint16_t ** tables_trellis = (const uint16_t **) tables_dev;
        const half **     tables_suh     = (const half **) ((char *) tables_dev + np * sizeof(void *));
        const half **     tables_svh     = (const half **) ((char *) tables_dev + 2 * np * sizeof(void *));
        ...
        static const int group_size_env = []() {
            const char * e = getenv("GGML_PAW_X3_MOE_GROUP_SIZE");
            return e ? atoi(e) : 0;
        }();
        const int concurrency = X3G_MIN(num_sms / X3M_SMS_PER_EXPERT, X3M_MAX_GROUPS);
        const int num_groups  = (int) X3G_MIN((int64_t) concurrency, num_active);
        const int group_size  = group_size_env > 0 ? group_size_env
                                                   : X3G_MIN(num_sms / num_groups, X3M_MAX_SMS_PER_EXPERT);

        ggml_cuda_pool_alloc<half>  xh(ctx.pool(), n * n_tokens);
        ggml_cuda_pool_alloc<half>  tsg(ctx.pool(), (size_t) num_groups * fused_rows * n);
        ggml_cuda_pool_alloc<half>  tsu(ctx.pool(), (size_t) num_groups * fused_rows * n);
        ggml_cuda_pool_alloc<half>  tig(ctx.pool(), (size_t) num_groups * fused_rows * ni);
        ggml_cuda_pool_alloc<half>  tiu(ctx.pool(), (size_t) num_groups * fused_rows * ni);
        ggml_cuda_pool_alloc<int>   locks(ctx.pool(), X3M_LOCK_INTS);
        CUDA_CHECK(cudaMemsetAsync(locks.get(), 0, X3M_LOCK_INTS * sizeof(int), stream));

        x3_cast_f32_f16_kernel<<<((size_t) n * n_tokens + 255) / 256, 256, 0, stream>>>(
            xh.get(), (const float *) x->data, (int) (n * n_tokens));

        char shp[80];
        snprintf(shp, sizeof(shp), " n=%lld ni=%lld tok=%lld active=%lld", (long long) n, (long long) ni,
                 (long long) n_tokens, (long long) num_active);
        paw_timed(stream, std::string("x3_moe") + shp, [&]() {
            x3m_moe_kernel<<<dim3(group_size, 1, num_groups), X3M_BLOCK_THREADS, X3G_SMEM_MAX, stream>>>(
                xh.get(), tsg.get(), tsu.get(), tig.get(), tiu.get(), (float *) dst->data,
                tables_trellis, tables_suh, tables_svh, tables_k,
                expert_count_dev, token_sorted_dev, weight_sorted.get(),
                (int) n, (int) ni, (int) n_expert, fused_rows, locks.get());
            CUDA_CHECK(cudaGetLastError());
        });

        // debug: dump the group-0 temp buffers of a single-expert launch for stage-by-stage comparison
        ...
        if (moe_dump && !moe_dumped && num_active == 1) {
            ...
            auto dump = [&](const char * name, const void * dev_ptr, size_t bytes) {
                ...
                CUDA_CHECK(cudaMemcpy(host.data(), dev_ptr, bytes, cudaMemcpyDeviceToHost));
                std::string path = std::string(moe_dump) + "/" + name;
            ...
            if (fm) {
                fprintf(fm, "n %lld ni %lld rows %lld expert %lld Kg %d Ku %d Kd %d groups %d group_size %d\n",
                        (long long) n, (long long) ni, (long long) rows, (long long) e0,
                        kmeta[0][2 * e0], kmeta[1][2 * e0], kmeta[2][2 * e0], num_groups, group_size);
                fclose(fm);
            }
            dump("xh.f16", xh.get(), n * n_tokens * 2);
            dump("token_sorted.i32", token_sorted_dev, n_rows * 4);
            dump("weight_sorted.f32", weight_sorted.get(), n_rows * 4);
            dump("tsg.f16", tsg.get(), rows * n * 2);
            dump("tsu.f16", tsu.get(), rows * n * 2);
            dump("tig.f16", tig.get(), rows * ni * 2);
            dump("tiu.f16", tiu.get(), rows * ni * 2);
            dump("dst.f32", dst->data, ggml_nbytes(dst));
            static const char * pn[3] = { "gate", "up", "down" };
            for (int p = 0; p < 3; ++p) {
                const int64_t pin  = P(p, 2)->ne[0];
                const int64_t pout = P(p, 3)->ne[0];
                const int64_t k    = kmeta[p][2 * e0];
                char nm[64];
                snprintf(nm, sizeof(nm), "%s_trellis.u16", pn[p]);
                dump(nm, (char *) P(p, 0)->data + (size_t) kmeta[p][2 * e0 + 1] * 2, (size_t) 16 * k * (pin / 16) * (pout / 16) * 2);
                snprintf(nm, sizeof(nm), "%s_suh.f16", pn[p]);
                dump(nm, (char *) P(p, 2)->data + e0 * P(p, 2)->nb[1], pin * 2);
                snprintf(nm, sizeof(nm), "%s_svh.f16", pn[p]);
                dump(nm, (char *) P(p, 3)->data + e0 * P(p, 3)->nb[1], pout * 2);
            }
        }
    }
    ...
    for (int64_t e = 0; e < n_expert; ++e) {
        ...
        if (cnt > fused_rows) {
            ...
            ggml_cuda_pool_alloc<float> ge(ctx.pool(), cnt * ni);
            ggml_cuda_pool_alloc<float> ue(ctx.pool(), cnt * ni);
            ggml_cuda_pool_alloc<float> ye(ctx.pool(), cnt * n);
            get_rows_cuda(x->data, GGML_TYPE_F32, token_sorted_dev + beg, xe.get(), GGML_TYPE_F32,
                n, x->nb[1], x->nb[2], x->nb[3],
                cnt, 1, 1, sizeof(int32_t), cnt * sizeof(int32_t), cnt * sizeof(int32_t),
                n * fs, cnt * n * fs, cnt * n * fs, stream);
            CUDA_CHECK(cudaGetLastError());
            x3_expert_mm(ctx, P(0, 0), kmeta[0], P(0, 2), P(0, 3), e, xe.get(), ge.get(), cnt);
            x3_expert_mm(ctx, P(1, 0), kmeta[1], P(1, 2), P(1, 3), e, xe.get(), ue.get(), cnt);
            const int64_t nel = cnt * ni;
            x3m_swiglu_kernel<<<(nel + 255) / 256, 256, 0, stream>>>(ge.get(), ue.get(), (int) nel);
            x3_expert_mm(ctx, P(2, 0), kmeta[2], P(2, 2), P(2, 3), e, ge.get(), ye.get(), cnt);
            x3m_scatter_add_kernel<<<dim3((int) ((n + 255) / 256), (int) cnt), 256, 0, stream>>>(
                (float *) dst->data, ye.get(), token_sorted_dev + beg, weight_sorted.get() + beg, (int) n);
            CUDA_CHECK(cudaGetLastError());
        }
        beg += cnt;
    }
